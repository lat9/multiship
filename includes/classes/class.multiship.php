...
class multiship extends base {
  ...
  //
  function _createOrderHeader ($order_info_array) {
    global $db;
    ...
    if ($this->selected) {
      ...
      $db->Execute("UPDATE " . TABLE_ORDERS . " SET order_total = $order_total, order_tax = $order_tax, shipping_method = '" . $this->shipping_method . "' WHERE orders_id = $orders_id");
      ...
      foreach ($this->details as $address_id => $currentInfo) {
        ...
        foreach ($currentInfo['totals'] as $currentTotal) {
          $sql = array ( 'orders_multiship_id' => $multiship_id,
                         'orders_id' => $orders_id,
  ...
  function _createOrderFixupTotal ($orders_totals_array) {
    ...
    if (is_array($this->totals) && isset($this->totals[$orders_totals_array['class']])) {
      ...
      $db->Execute("UPDATE " . TABLE_ORDERS_TOTAL . " SET text = '" . $currencies->format($currentTotal) . "', value = " . $currentTotal . " WHERE orders_total_id = $insert_id");
    }
  }
  ...
  //
  function _createOrderAddAttributes($products_attributes_array) {
    if ($this->selected) {
  ...
  function _insertAttributesText($order) {
    if ($this->selected) {
      foreach ($this->details as $address_id => &$currentInfo) {
  ...
  // order's processing.
  //
  function _saveEmailHeader ($text_email, $html_email) {
  ...
  function _prepare() {
    ...
    if ($this->selected) {
      ...
      foreach ($this->cart as $address_id => $products) {
        ...
        if (!is_array($shipping_quote) || sizeof($shipping_quote) == 0) {
