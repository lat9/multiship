...
//      if (($key != zen_session_name()) && ($key != 'error') && (!in_array($key, $exclude_array))) $get_url .= $key . '=' . $value . '&';  XSS Flaw Patch
...
//    }
  ...
  function zen_remove_order($order_id, $restock = false) {
    global $db, $zco_notifier;  //-multiship-1/xxx
    if ($restock == 'on') {
    ...
    $zco_notifier->notify('NOTIFY_FUNCTION_GENERAL_REMOVE_ORDER', array ('orders_id' => $order_id));  //-multiship-2/xxx
  }
